...
describe('v3beta1.TranslationServiceClient', () => {
    ...
    describe('batchTranslateText', () => {
        it('invokes batchTranslateText without error', done => {
            ...
            client.batchTranslateText(request).then((responses: [Operation]) => {
    ...
    describe('createGlossary', () => {
        it('invokes createGlossary without error', done => {
            ...
            client.createGlossary(request).then((responses: [Operation]) => {
                const operation = responses[0];
                return operation? operation.promise() : {};
            }).then((responses: [Operation]) => {
                assert.deepStrictEqual(responses[0], expectedResponse);
    ...
    describe('deleteGlossary', () => {
        it('invokes deleteGlossary without error', done => {
            ...
            client.deleteGlossary(request).then((responses: [Operation]) => {
                const operation = responses[0];
                return operation? operation.promise() : {};
            }).then((responses: [Operation]) => {
                assert.deepStrictEqual(responses[0], expectedResponse);
    ...
    describe('listGlossariesStream', () => {
        it('invokes listGlossariesStream without error', done => {
            ...
            const stream = client.listGlossariesStream(request, {}).on('data', (response: {}) =>{
                assert.deepStrictEqual(response, expectedResponse);
                done();
